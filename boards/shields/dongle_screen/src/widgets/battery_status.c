...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < ZMK_SPLIT_CENTRAL_PERIPHERAL_COUNT + SOURCE_OFFSET; i++) {
        ...
        lv_canvas_set_buffer(image_canvas, battery_image_buffer[i], 51, 3, LV_IMG_CF_TRUE_COLOR);

        lv_obj_align(image_canvas, LV_ALIGN_BOTTOM_MID, -30 +(i * 60), -4);
        lv_obj_align(battery_label, LV_ALIGN_TOP_MID, -30 +(i * 60), 0);
